...
server.post<{ Body: Entry }>("/create/", async (req, reply) => {
  ...
  newEntryBody.due_at ? (newEntryBody.due_at = new Date(req.body.due_at)) : (newEntryBody.due_at = new Date());
  try {
...
server.put<{ Params: { id: string }; Body: Entry }>("/update/:id", async (req, reply) => {
  ...
  updatedEntryBody.due_at
    ? (updatedEntryBody.due_at = new Date(req.body.due_at))
    : (updatedEntryBody.due_at = new Date());
